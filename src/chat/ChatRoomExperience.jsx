...
export const ChatRoomLanding = ({ onEnterRoom }) => {
  ...
  const onJoin = async () => {
    ...
    if (!isRoomCodeValid(code)) {
      setError(chatCopy.invalidRoomCode);
      return;
    }
      ...
      <div className="mx-auto max-w-[70rem] px-4 py-6 sm:px-8 sm:py-10">
        <div className="rounded-3xl border border-slate-200 bg-white px-5 py-6 shadow-sm sm:px-8 sm:py-8">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 sm:text-2xl">{chatCopy.title}</h2>
              <p className="mt-2 text-sm font-medium leading-7 text-slate-600">{chatCopy.description}</p>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
            ...
            >
              {busyAction === 'join' ? chatCopy.joiningRoom : chatCopy.joinRoom}
            </button>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <input
              ...
              placeholder={chatCopy.roomCodePlaceholder}
            />
          </div>
...
export const ChatRoomPanel = ({ roomCode, bootstrap, onBackToLanding }) => {
  const [roomId, setRoomId] = useState('');
  const [expiresAt, setExpiresAt] = useState('');
  const [messages, setMessages] = useState([]);
  const [bootState, setBootState] = useState('loading');
  const [statusText, setStatusText] = useState(chatCopy.connecting);
  const [error, setError] = useState('');
  ...
  const roomStateRef = useRef({ roomId: '', isExpired: false, isLeaving: false });
  const exitControllerRef = useRef(null);
  ...
  useEffect(() => {
    exitControllerRef.current = createRoomExitController({
      getState: () => roomStateRef.current,
      leaveRoomKeepalive,
      unsubscribeRealtime,
    });
  }, [unsubscribeRealtime]);
  ...
  useEffect(() => {
    const handlePageExit = () => {
      exitControllerRef.current?.handlePageExit();
    };
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      setError('');
      setStatusText(chatCopy.connecting);
      setMessages([]);
      setRoomId('');
      setExpiresAt('');
      setNicknameState('');
      ...
      exitControllerRef.current?.reset();
      ...
      try {
        ...
        if (cancelled) return;
        setError(toChatErrorMessage(err, chatCopy.roomNotFound));
        setBootState('error');
        logChatEvent('error', null, {
          stage: 'room_init',
          room_code: roomCode,
    ...
    const channel = subscribeMessages(
      ...
      async (status) => {
        if (status === 'CHANNEL_ERROR') {
          setStatusText(chatCopy.connectionRetrying);
          return;
        }
        if (status === 'TIMED_OUT') {
          setStatusText(chatCopy.connectionTimedOut);
          return;
        }
        if (status === 'SUBSCRIBED') {
          setStatusText(chatCopy.connectionHealthy);
  ...
  }, [appendMessage, refreshMessages, roomId, unsubscribeRealtime]);
  ...
  useEffect(() => {
    ...
    exitControllerRef.current?.markHandled();
    setStatusText(chatCopy.roomExpiredStatus);
    setError(chatCopy.roomExpiredError);
    unsubscribeRealtime();
    leaveRoom(roomId).catch(() => {});
    logChatEvent('expired', roomId, { room_code: roomCode }).catch(() => {});
  }, [isExpired, roomCode, roomId, unsubscribeRealtime]);

  const onSaveNickname = async () => {
    const normalized = normalizeNickname(nicknameDraft);
    if (!normalized || normalized.length > 20) {
      setError(chatCopy.nicknameInvalid);
      return;
    }
    if (!roomId) return;
    ...
    return (
      <div className="h-full bg-[#F8FAFC] p-4 sm:p-8">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 text-center text-sm font-semibold text-slate-500">
          {chatCopy.enteringRoom} {roomCode} ...
        </div>
      </div>
    );
  }
        ...
        <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm sm:px-5">
          <div className="grid grid-cols-2 items-center gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-between sm:gap-3">
            ...
            <div className="col-span-2 inline-flex items-center justify-center gap-2 rounded-xl bg-slate-100 px-3 py-2 sm:col-span-1 sm:justify-start">
              ...
              <span className="text-[11px] font-semibold text-blue-700">{copied ? chatCopy.copied : ''}</span>
            </div>
            ...
            <div className="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700 sm:text-sm">
              <Timer size={14} />
              {isExpired ? chatCopy.roomExpiredStatus : `${chatCopy.remainingPrefix} ${formatRemainingTime(remainingMs)}`}
            </div>
            ...
            >
              <LogOut size={14} />
              {leaving ? chatCopy.exitingRoom : chatCopy.exitRoom}
            </button>
          </div>
                ...
                {messages.map((item) => (
                  <article key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-3.5 sm:p-4">
                    <div className="mb-1.5 flex items-center gap-2 text-xs font-bold text-slate-500">
                      <UserRound size={13} />
                      <span>{item.nickname || chatCopy.anonymousUser}</span>
          ...
          <div className="border-t border-slate-200 bg-slate-50 p-3 sm:p-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
              <textarea
                ...
                placeholder={isExpired ? chatCopy.sendDisabledPlaceholder : chatCopy.sendPlaceholder}
              ...
              >
                {isSending ? chatCopy.sending : chatCopy.send}
              </button>
            </div>

            <div className="mt-2 flex items-center justify-between text-xs text-slate-500">
              <span>{nickname ? `${chatCopy.nicknamePrefix}${nickname}` : chatCopy.nicknameMissing}</span>
        ...
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-900/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
            ...
            <input
              ...
              placeholder={chatCopy.nicknamePlaceholder}
            />
            ...
            >
              {savingNickname ? chatCopy.savingNickname : chatCopy.confirmEnter}
            </button>
          </div>
        </div>
      )}
