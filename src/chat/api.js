...
export const toChatErrorMessage = (error, fallback = '请求失败，请稍后重试') => {
  ...
  if (raw.includes('ANON_AUTH_DISABLED')) {
    return '匿名登录未启用。请到 Supabase Dashboard -> Authentication -> Providers 开启 Anonymous。';
  }
  ...
  if (raw.includes('INVALID_NICKNAME')) return '昵称需为 1-20 个字符。';
  if (raw.includes('NICKNAME_REQUIRED')) return '请先设置昵称。';
  if (raw.includes('INVALID_MESSAGE_LENGTH')) return '消息长度需在 1-500 个字符。';
...
const invokeKeepaliveRpc = async (rpcName, payload) => {
...
export const leaveRoomKeepalive = async (roomId) => {
  if (!roomId) return false;
  return invokeKeepaliveRpc('leave_room', { p_room_id: roomId });
};
