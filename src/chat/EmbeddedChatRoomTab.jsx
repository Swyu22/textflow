          ...
          <div className="border-t border-slate-200 bg-slate-50 p-3 sm:p-4">
            <div className="flex items-end gap-2">
              ...
              <button
                ...
                onClick={onSend}
                disabled={isExpired || nicknameModalOpen || isSending}
                className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-black text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isSending ? '发送中...' : '发送'}
              </button>
            </div>
      ...
      {nicknameModalOpen && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-900/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
            ...
            <p className="mt-1 text-sm font-medium text-slate-600">昵称仅在当前房间显示，不会展示账号身份信息。</p>
            <input
              type="text"
              value={nicknameDraft}
              maxLength={20}
